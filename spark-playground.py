...
def load_data(path, df_name):
	...
	pct_drop=float(((count_before-count_after)/count_before)*100)
...
def transform_meta(meta_df):
	...
	meta_df = meta_df.withColumn('Day of Week(Sunday=0)', get_weekday(col("event_date_local")))
	...
	meta_df = meta_df.withColumn('Game Time(day,afternoon,night)', get_game_time(col("event_date_local")))
...
def join_dfs(transformed_inventory, transformed_meta, transformed_performance):
	...
	joined_df = joined_df.join(transformed_performance, joined_df.Concatenated_joined == transformed_performance.Concatenated_perf, 'left_outer')

	print("Count of successful concat joins: %s" %joined_df.where(col("Concatenated_perf").isNull()).count())
...
def transform_days_diff(joined_df):
	...
	transformed = joined_df.withColumn('Time_to_event(days)', udf_get_days_diff(joined_df.timestamp, joined_df.event_date_utc)).cache()
	
	transformed = transformed.orderBy('Time_to_event(days)', ascending=False).cache()
...
if __name__ == "__main__":
	...
	performance_paths = {"v3":"s3a://kartees-hadoop-data/event_metadata_v2_hadoop_v3", "sample-set":"s3a://kartees-hadoop-data/performance_sample_set"}
...
#-------------------------------------------------------------------------------
	...
	# transformed_days_diff.show()

	script_step(9)

	categorical_columns = ['Home_Team_Id', 'Away_Team_Id', 'sectionId', 'Day of Week(Sunday=0)','Game Time(day,afternoon,night)']
	#categorical_columns = ['Game Time(day,afternoon,night)']

	transformed_categorical = transform_categorical(transformed_days_diff, categorical_columns).cache()

	print("Final count: %s" %transformed_categorical.count())
	#transformed_categorical.select('sectionId','sectionId_indexed','sectionId_indexed_encoded','cat_features' ).show()
